...
public class ChessState : Object
{
    ...
    /* Cache location of kings to avoid O(64) search */
                                 ...
                                 bool apply = true, bool test_check = true)
    {
        ...
        var old_king_location = -1;
        var old_opponent_king_location = -1;
        ...
        if (victim != null)
        {
            ...
            {
                old_opponent_king_location = king_locations[opponent_color];
                king_locations[opponent_color] = -1;
            }
        }
        if (is_promotion)
        ...
        {
            old_king_location = king_locations[color];
        ...
        if (!apply || !result)
        {
            ...
            if (victim != null)
            {
                ...
                if (victim.type == PieceType.KING)
                    king_locations[opponent_color] = old_opponent_king_location;
            }
            if (rook_start >= 0)
            {
    ...
    public bool get_positions_threatening_king (ChessPlayer player, out int[] rank, out int[] file)
    {
        ...
        /* Optimization: Use cached king location */
